...
with st.sidebar:
    ...
    st.caption('To inference the model you need to upload a dataframe in csv format with douze columns/features')
    ...
    st.divider() 
    st.caption("<p style = 'text-align:center'>Developed by Noé Fouotsa</p>", unsafe_allow_html = True)
...
if st.session_state.clicked[1]:
    ...
    if uploaded_file is not None:
        ...
        pred = pd.DataFrame(pred, columns = ['No risk', 'Risk of early diabetes'])
